...
class AuthorsRegiterTest(AuthorsBaseTest):
    ...
    def form_field_test_with_calback(self, callback):
    ...
    def test_empty_first_name_error_message(self):
        ...
        self.form_field_test_with_calback(callback)
    ...
    def test_empty_username_error_message(self):
        def callback(form):
            user_name_field = self.get_by_placeholder(form, 'Your username')
            user_name_field.send_keys(' ')
    ...
    def test_invalid_email_error_message(self):
        def callback(form):
            first_name_field = self.get_by_placeholder(form, 'Your e-mail')
            first_name_field.send_keys('email@invalid')
            first_name_field.send_keys(Keys.ENTER)
            form = self.get_form()
    ...
    def test_passwords_do_not_match_error_message(self):
        def callback(form):
            ...
            first_name_field = self.get_by_placeholder(form, 'Repeat your password')
            first_name_field.send_keys('123456')
            first_name_field.send_keys('654321')
            first_name_field.send_keys(Keys.ENTER)
            form = self.get_form()
